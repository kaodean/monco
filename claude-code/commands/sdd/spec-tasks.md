...
- Otherwise: Stop if requirements/design missing or unapproved with message:
  "Run `/sdd:spec-requirements` and `/sdd:spec-design` first, or use `-y` flag to auto-approve"
- If tasks.md exists: Prompt [o]verwrite/[m]erge/[c]ancel

**Context Loading (Full Paths)**:

1. `.sdd/specs/$1/requirements.md` - Feature requirements (EARS format)
2. `.sdd/specs/$1/design.md` - Technical design document
3. `.sdd/steering/` - Project-wide guidelines and constraints:
   - **Core files (always load)**:
     - @.sdd/steering/product.md - Business context, product vision, user needs
     - @.sdd/steering/tech.md - Technology stack, frameworks, libraries
     - @.sdd/steering/structure.md - File organization, naming conventions, code patterns
   - **Custom steering files** (load all EXCEPT "Manual" mode in `AGENTS.md`):
     - Any additional `*.md` files in `.sdd/steering/` directory
     - Examples: `api.md`, `testing.md`, `security.md`, etc.
   - (Task planning benefits from comprehensive context)
4. `.sdd/specs/$1/tasks.md` - Existing tasks (only if merge mode)
...
- Generate `.sdd/specs/$1/tasks.md` using the exact numbering format above
...
```bash
/sdd:spec-impl $1          # Execute all pending tasks
/sdd:spec-impl $1 1.1      # Execute specific task
/sdd:spec-impl $1 1,2,3    # Execute multiple tasks
```
